...
export function AIModelSettings() {
  ...
  const syncToCloud = useCallback(async (payloadOverrides = {}) => {
    ...
    setIsSaving(false);
  }, [model, temperature, swarmMode, enforcer, systemPrompt]);
        ...
        <CardContent className="relative z-10 space-y-8 pt-8">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div 
                 onClick={() => toggleModel('gemini-flash')}
                 className={cn(
                   "relative p-6 rounded-2xl border cursor-pointer transition-all duration-300 flex flex-col gap-4 overflow-hidden",
                   model === 'gemini-flash' ? "bg-fuchsia-500/10 border-fuchsia-500/50 shadow-[0_0_30px_rgba(192,38,211,0.15)] transform scale-[1.02] ring-1 ring-fuchsia-500" : "bg-black/40 border-zinc-800 hover:border-zinc-700 hover:bg-zinc-900/50"
                 )}
               >
                  {model === 'gemini-flash' && <div className="absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-fuchsia-500 to-transparent opacity-50" />}
                  <div className="flex justify-between items-start">
                     <Cpu className={cn("w-6 h-6", model === 'gemini-flash' ? "text-fuchsia-400" : "text-zinc-500")} />
                     <span className="px-2 py-0.5 bg-zinc-800 text-[9px] font-black uppercase tracking-widest text-white rounded">Default API</span>
                  </div>
                  <div>
               ...
               <div 
                 onClick={() => toggleModel('gemini-pro')}
                 className={cn(
                   "relative p-6 rounded-2xl border cursor-pointer transition-all duration-300 flex flex-col gap-4 overflow-hidden",
                   model === 'gemini-pro' ? "bg-red-500/10 border-red-500/50 shadow-[0_0_30px_rgba(239,68,68,0.15)] transform scale-[1.02] ring-1 ring-red-500" : "bg-black/40 border-zinc-800 hover:border-zinc-700 hover:bg-zinc-900/50"
                 )}
               >
                  {model === 'gemini-pro' && <div className="absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-red-500 to-transparent opacity-50" />}
                  <div className="flex justify-between items-start">
                     <DatabaseZap className={cn("w-6 h-6", model === 'gemini-pro' ? "text-red-500" : "text-zinc-500")} />
