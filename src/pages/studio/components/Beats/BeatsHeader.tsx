...
import { Activity, Sparkles, Heart, ChevronRight, RefreshCw, Cpu, BrainCircuit } from 'lucide-react';

interface BeatsHeaderProps {
  session: string;
  episode: string;
  isContextLocked: boolean;
  viewMode: 'grid' | 'timeline' | 'mastermind';
  setViewMode: (mode: 'grid' | 'timeline' | 'mastermind') => void;
  onGenerate: () => void;
...
export const BeatsHeader: React.FC<BeatsHeaderProps> = ({
  session,
  episode,
  isContextLocked,
  viewMode,
  ...
  return (
    <div className="relative group">
      ...
      <div className="absolute -inset-1 bg-gradient-to-r from-studio/20 via-cyan-500/10 to-studio/20 rounded-[2rem] blur-xl opacity-20 group-hover:opacity-40 transition-opacity duration-1000" />
      ...
      <div className="relative flex flex-col lg:flex-row items-center justify-between p-6 bg-[#050505]/80 backdrop-blur-3xl border border-studio/20 rounded-[2rem] shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-8 z-10 w-full lg:w-auto">
          <div className="relative">
            <div className="w-14 h-14 rounded-2xl bg-studio/10 border border-studio/30 flex items-center justify-center shadow-[0_0_20px_rgba(6,182,212,0.2)] group/icon overflow-hidden">
              ...
              <BrainCircuit className="w-7 h-7 text-studio relative z-10 animate-pulse-slow" />
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="px-2 py-0.5 rounded-full bg-studio/10 border border-studio/20 flex items-center gap-1.5">
                ...
                <span className="text-[8px] font-black uppercase tracking-widest text-studio/80">
                  {isGenerating ? 'Synthesizing' : 'Beat_Locked'}
                </span>
              </div>
            </div>
            <div className="flex items-center gap-2 mt-2">
              <Cpu className="w-3 h-3 text-studio/40" />
              <p className="text-[9px] font-black text-studio/40 uppercase tracking-[0.4em]">Structural Intelligence Engine V4.1 // Core_Sync_Active</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6 mt-6 lg:mt-0 z-10 w-full lg:w-auto justify-between lg:justify-end">
          ...
          <div className="hidden sm:flex items-center gap-4 px-4 py-2 bg-black/60 border border-zinc-800/50 rounded-2xl backdrop-blur-md">
            ...
            <div className={cn(
              "flex items-center gap-2 px-3 py-1 rounded-xl border transition-all duration-500 ml-2",
              isContextLocked ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-zinc-800/50 border-zinc-700/50 text-zinc-500"
            )}>
              <Activity className="w-3 h-3 animate-pulse" />
              <span className="text-[8px] font-black uppercase tracking-widest">
                {isContextLocked ? "Manifested" : "Pending"}
              </span>
            </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex bg-black/60 p-1.5 rounded-[1.25rem] border border-zinc-800/50 backdrop-blur-md mr-1">
              {(['grid', 'timeline', 'mastermind'] as const).map((mode) => (
                <Button
                  ...
                  className={cn(
                    "h-8 px-4 text-[8px] font-black uppercase tracking-[0.2em] transition-all duration-300 rounded-lg",
                    viewMode === mode 
                      ? "bg-studio/20 text-studio border border-studio/20 shadow-[0_0_10px_rgba(6,182,212,0.1)]" 
            ...
            >
              {isGenerating ? <RefreshCw className="w-4 h-4 animate-spin mr-2" /> : <Sparkles className="w-4 h-4 mr-2 text-studio group-hover/btn:scale-110 transition-transform" />}
              Synthesize Beats
            </Button>
            ...
            >
              <span className="relative z-10 flex items-center gap-2">
                Next: Script <ChevronRight className="w-4 h-4 group-hover/next:translate-x-1 transition-transform" />
              </span>
