      ...
      <div className="relative flex flex-col lg:flex-row items-center justify-between p-5 md:p-6 bg-[#050505]/80 backdrop-blur-3xl border border-studio/20 rounded-[2rem] shadow-2xl overflow-hidden gap-6 lg:gap-0">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 z-10 w-full lg:w-auto">
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
            ...
            {/* Session Navigation */}
            <div className="flex w-full sm:w-auto justify-center bg-black/40 border border-white/5 p-1 rounded-2xl backdrop-blur-xl">
              {[1, 2, 3, 4].map((s) => (
                <button
                  key={s}
                  onClick={() => setActiveSession(s)}
                  className={cn(
                    "relative h-9 flex-1 sm:flex-none px-4 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all duration-300",
                    activeSession === s 
