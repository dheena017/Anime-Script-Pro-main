...
export function ProfilePage() {
    ...
    const fetchEverything = async () => {
      ...
      try {
        const [profileRes, balanceRes, assetsRes, favRes, promptsRes, charsRes, settingsRes] = await Promise.all([
          fetch(`http://localhost:8000/api/profiles/${user.id}`),
          fetch(`http://localhost:8000/api/balances/${user.id}`),
          fetch(`http://localhost:8000/api/assets/${user.id}`),
          fetch(`http://localhost:8000/api/favorites/${user.id}`),
          fetch(`http://localhost:8000/api/library/prompts/${user.id}`),
          fetch(`http://localhost:8000/api/library/characters/${user.id}`),
          fetch(`http://localhost:8000/api/settings/${user.id}`)
        ]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await fetch(`http://localhost:8000/api/profiles/${user.id}`, {
        method: 'POST',
      ...
      await fetch(`http://localhost:8000/api/settings/${user.id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const addPrompt = async () => {
    ...
    const res = await fetch(`http://localhost:8000/api/library/prompts`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const addDNA = async () => {
    ...
    const res = await fetch(`http://localhost:8000/api/library/characters`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
